...
import { useConfigureMinter } from './useConfigureMinter';
...
describe("Unit tests of 'useConfigureMinter' hook", () => {
  ...
  test('tx succeeds w/o any errors', async () => {
    ...
    const minterAllowedAmount = 10000;
    ...
    (useWriteContract as Mock)
      ...
      .mockReturnValueOnce({
        // right after calling `configureMinter`
        data: txHash,
        isPending: true,
        error: writeError,
        writeContract,
    ...
    const { result, rerender } = renderHook(() => useConfigureMinter());

    expect(result.current.configureMinter).toBeDefined();
    ...
    await act(async () => {
      await result.current.configureMinter!({
        minter,
        minterAllowedAmount,
      });
      rerender();
    ...
    expect(simulateContract).toHaveBeenCalledWith(
      {},
      {
        ...
        args: [minter, 10000000000000000000000n],
        account: userAddress,
      },
    );
  ...
  test('tx simulation fails', async () => {
    ...
    const { result } = renderHook(() => useConfigureMinter());
  ...
  test('hook not ready: `contractAbi` undefined', async () => {
    ...
    expect(result.current.configureMinter).toBeUndefined();
