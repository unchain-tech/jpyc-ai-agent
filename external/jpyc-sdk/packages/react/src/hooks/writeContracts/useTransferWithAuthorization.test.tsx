...
import { useTransferWithAuthorization } from './useTransferWithAuthorization';
...
describe("Unit tests of 'useTransferWithAuthorization' hook", () => {
  ...
  test('tx succeeds w/o any errors', async () => {
    ...
    const from = '0x90F79bf6EB2c4f870365E785982E1f101E93b906';
    const to = '0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC';
    const value = 10000;
    const validAfter = Uint256.from('0');
    const validBefore = Uint256.from('3600');
    const nonce = '0x456';
    ...
    (useWriteContract as Mock)
      ...
      .mockReturnValueOnce({
        // right after calling `transferWithAuthorization`
        data: txHash,
        isPending: true,
        error: writeError,
        writeContract,
    ...
    const { result, rerender } = renderHook(() => useTransferWithAuthorization());

    expect(result.current.transferWithAuthorization).toBeDefined();
    ...
    await act(async () => {
      await result.current.transferWithAuthorization!({
        from,
        to,
        value,
        validAfter,
    ...
    expect(simulateContract).toHaveBeenCalledWith(
      {},
      {
        ...
        args: [from, to, 10000000000000000000000n, validAfter, validBefore, nonce, v, r, s],
        account: userAddress,
      },
    );
  ...
  test('tx simulation fails', async () => {
    ...
    const { result } = renderHook(() => useTransferWithAuthorization());
  ...
  test('hook not ready: `contractAbi` undefined', async () => {
    ...
    expect(result.current.transferWithAuthorization).toBeUndefined();
