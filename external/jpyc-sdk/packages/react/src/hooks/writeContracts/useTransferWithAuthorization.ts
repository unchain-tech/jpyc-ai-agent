...
/** Type for response from `useTransferWithAuthorization` hook. */
type UseTransferWithAuthorizationResponse = UseWriteContractResponse & {
...
/**
 * Configures & exposes `transferWithAuthorization` method of JPYC contract.
 *
 * @returns Callable object (& related states)
 */
export function useTransferWithAuthorization(): UseTransferWithAuthorizationResponse {
   ...
   * @param s s of ECDSA
   */
  const transferWithAuthorization = async ({
    from,
    to,
    value,
    validAfter,
    ...
    try {
      ...
      const { request } = await simulateContract(wagmiConfig, {
        ...
        functionName: CONTRACT_FUNC_NAMES.transferWithAuthorization,
        args: [
          from,
          to,
  ...
  return {
    transferWithAuthorization: isReady ? transferWithAuthorization : undefined,
    isReady,
    isLoading,
    isSuccess,
    error,
