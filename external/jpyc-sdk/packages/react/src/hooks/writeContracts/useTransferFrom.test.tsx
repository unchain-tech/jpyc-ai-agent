...
import { useTransferFrom } from './useTransferFrom';
...
describe("Unit tests of 'useTransferFrom' hook", () => {
  ...
  test('tx succeeds w/o any errors', async () => {
    ...
    (useWriteContract as Mock)
      ...
      .mockReturnValueOnce({
        // right after calling `transferFrom`
        data: txHash,
        isPending: true,
        error: writeError,
        writeContract,
    ...
    const { result, rerender } = renderHook(() => useTransferFrom());

    expect(result.current.transferFrom).toBeDefined();
    ...
    await act(async () => {
      await result.current.transferFrom!({
        from,
        to,
        value,
      });
    ...
    expect(simulateContract).toHaveBeenCalledWith(
      {},
      {
        ...
        args: [from, to, 10000000000000000000000n],
        account: userAddress,
      },
    );
  ...
  test('tx simulation fails', async () => {
    ...
    const { result } = renderHook(() => useTransferFrom());
  ...
  test('hook not ready: `contractAbi` undefined', async () => {
    ...
    expect(result.current.transferFrom).toBeUndefined();
