...
import { useDecreaseAllowance } from './useDecreaseAllowance';
...
describe("Unit tests of 'useDecreaseAllowance' hook", () => {
  ...
  test('tx succeeds w/o any errors', async () => {
    ...
    (useWriteContract as Mock)
      ...
      .mockReturnValueOnce({
        // right after calling `decreaseAllowance`
        data: txHash,
        isPending: true,
        error: writeError,
        writeContract,
    ...
    const { result, rerender } = renderHook(() => useDecreaseAllowance());

    expect(result.current.decreaseAllowance).toBeDefined();
    ...
    await act(async () => {
      await result.current.decreaseAllowance!({
        spender,
        decrement,
      });
      rerender();
    ...
    expect(simulateContract).toHaveBeenCalledWith(
      {},
      {
        ...
        args: [spender, 10000000000000000000000n],
        account: userAddress,
      },
    );
  ...
  test('tx simulation fails', async () => {
    ...
    const { result } = renderHook(() => useDecreaseAllowance());
  ...
  test('hook not ready: `contractAbi` undefined', async () => {
    ...
    expect(result.current.decreaseAllowance).toBeUndefined();
