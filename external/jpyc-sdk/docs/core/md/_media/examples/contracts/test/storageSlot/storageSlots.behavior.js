...
function usesOriginalStorageSlotPositions({ version, accounts }) {
  describe('uses original storage slot positions', () => {
    ...
    const [mintAllowance, minted, transferred, allowance] = [1000, 100, 30, 10];
    const [
      owner,
      minterAdmin,
      pauser,
    ...
    beforeEach(async () => {
      ...
      var { v, r, s } = fromRpcSig(signature);
      await proxyAsFiatToken.transferWithAuthorization(
        from,
        to,
        transferred,
        0,
    ...
    it('retains original storage slots 0 through 571', async () => {
      ...
      expect(parseAddress(parseBoolAddress(slots[51]).slice(2))).to.equal(pauser); // pauser
      ...
      const DOMAIN_SEPARATER = await domainSeparator(name, '1', 1337, proxy.address);
      ...
      expect(parseAddress(parseBoolUint8Address(slots[515]).slice(4))).to.equal(minterAdmin); // minterAdmin
      expect(parseUint(parseBoolUint8Address(slots[515]).slice(2, 4)).toNumber()).to.equal(
        decimals,
      );
    ...
    it('retains original storage slots for balances mapping', async () => {
      // balance[alice]
      let v = parseInt(await readSlot(proxy.address, addressMappingSlot(alice, 516)), 16);
      expect(v).to.equal(minted - transferred);

      // balances[bob]
      v = parseInt(await readSlot(proxy.address, addressMappingSlot(bob, 516)), 16);
      expect(v).to.equal(transferred);
    });

    it('retains original storage slots for allowed mapping', async () => {
      // allowed[alice][bob]
      let v = await readSlot(proxy.address, address2MappingSlot(alice, bob, 517));
      expect(v).to.equal('');
      // allowed[alice][charlie]
      v = parseInt(await readSlot(proxy.address, address2MappingSlot(alice, charlie, 517)), 16);
    ...
    it('retains original storage slots for minterAllowed mapping', async () => {
      ...
      let v = parseInt(await readSlot(proxy.address, addressMappingSlot(minter, 519)), 16);
...
function address2MappingSlot(addr, addr2, pos) {
  return web3.utils.keccak256('0x' + encodeAddress(addr2) + addressMappingSlot(addr, pos).slice(2));
}
