...
function shouldBehaveLikeFiatTokenV1(
  ...
  describe('mint', function () {
    ...
    describe('when the requested account is minter', function () {
      it('mint (full amount)', async function () {
        ...
        expect(await this.token.totalSupply()).to.be.bignumber.equal(initialSupply.add(value));
      ...
      it('mint (partical amount)', async function () {
        ...
        expect(await this.token.minterAllowance(minter)).to.be.bignumber.equal(value.sub(amount));
      });
  ...
  describe('minterAllowance', function () {
    ...
    it('return minterAllowed', async function () {
      ...
      expect(await this.token.minterAllowance(anotherAccount)).to.be.bignumber.equal(value);
    });
  });
  ...
  describe('burn', function () {
    ...
    describe('when the requested account is minter', function () {
      ...
      it('burn (partical amount)', async function () {
        ...
        expect(await this.token.totalSupply()).to.be.bignumber.equal(initialSupply.addn(1));
        expect(await this.token.balanceOf(minter)).to.be.bignumber.equal(value.sub(amount));
      });
  ...
  describe('transferFrom', function () {
    ...
    it('when maximum approve', async function () {
      ...
      expect(await this.token.allowance(owner, recipient)).to.be.bignumber.equal(MAX_UINT256);
    });
  });
}
