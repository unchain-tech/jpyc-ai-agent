...
function shouldBehaveLikeAllowlistable(
    ...
    describe('when the requested account is allowlister', function () {
      ...
      describe('when allowlist unallowlisted account', function () {
        it('returns true', async function () {
          ...
          expect(await this.token.isAllowlisted(unAllowlisted)).to.be.equal(true);
        });
      });
  ...
  describe('allowlistable token', function () {
    describe('approve', function () {
      it('allows to approve over 100000 tokens when Allowlisted', async function () {
        ...
        expect(await this.token.allowance(initialHolder, unAllowlisted)).to.be.bignumber.equal(
          sendAmountAbove,
        );
      });
      ...
      it('allows to approve under 100000 tokens when unAllowlisted', async function () {
        ...
        expect(await this.token.allowance(unAllowlisted, initialHolder)).to.be.bignumber.equal(
          sendAmountBelow,
        );
      });
  ...
  describe('transfer', function () {
    it('allows to transfer over 100000 when allowlisted', async function () {
      ...
      expect(await this.token.balanceOf(initialHolder)).to.be.bignumber.equal('100');
      expect(await this.token.balanceOf(unAllowlisted)).to.be.bignumber.equal(sendAmountAbove);
    });
    ...
    it('allows to transfer under 100000 tokens when allowlisted and then unAllowlisted', async function () {
      ...
      expect(await this.token.balanceOf(initialHolder)).to.be.bignumber.equal('100');
      expect(await this.token.balanceOf(unAllowlisted)).to.be.bignumber.equal(sendAmountBelow);
    });
  ...
  describe('transfer from', function () {
    ...
    it('allows to transferfrom over 100000 tokens when sender is Allowlisted', async function () {
      await this.token.transferFrom(initialHolder, unAllowlisted, sendAmountAbove, {
        from: anotherAccount,
      });

      expect(await this.token.balanceOf(unAllowlisted)).to.be.bignumber.equal(sendAmountAbove);

      expect(await this.token.balanceOf(initialHolder)).to.be.bignumber.equal(new BN(100));
    });

    it('allows to transferfrom under 100000 tokens when sender is not Allowlisted', async function () {
      await this.token.unAllowlist(initialHolder, { from: allowlister });
      await this.token.transferFrom(initialHolder, unAllowlisted, sendAmountBelow, {
        from: anotherAccount,
      });
  ...
  describe('mint', function () {
    ...
    it('allows to mint over 100000 tokens when minter is allowlisted', async function () {
      ...
      expect(await this.token.balanceOf(unAllowlisted)).to.be.bignumber.equal(mintSupply);
    });
