...
const { shouldBehaveLikeAllowlistable } = require('../v2/Allowlistable.behavior');
...
contract('FiatTokenV2_proxy', function (accounts) {
  ...
  beforeEach(async function () {
    ...
    // upgrade to v2
    this.tokenImpleAtProxy.upgradeToAndCall(this.implementationV2.address, _dataV2, {
      from: owner,
    });
  ...
  it('check addresses are not null', async function () {
    console.log('Imlementation FiatTokenV1 deployed to:', this.implementationV1.address);
  ...
  describe('shouldBehaveLikeERC20', () => {
    shouldBehaveLikeERC20('FiatToken', initialSupply, initialHolder, recipient, anotherAccount);
  });
  ...
  describe('shouldBehaveLikeEIP2612', () => {
    shouldBehaveLikeEIP2612('EIP2612', name, initialHolder, recipient, pauser, blocklister, '2');
  });
