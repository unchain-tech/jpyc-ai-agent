...
function shouldBehaveLikeOwner(alice) {
  it('upgradeTo', async function () {
    ...
    expect(
      web3.utils.toChecksumAddress('0x' + data.replace(/^0x/, '').substring(24, 64)),
    ).to.be.equal(this.impOk.address);
  });
  ...
  it('upgradeToAndCall', async function () {
    await this.contract.functionCall(
      ...
      this.token.contract.methods
        .upgradeToAndCall(this.impOk.address, this.token.contract.methods.owner().encodeABI())
        .encodeABI(),
    );
